...
save_path = f"outputs/summaries_inc/{MODEL}-{CHUNK_SIZE}-inc.json"
...
def compress(response, summary, chunk, templates, summary_len, word_limit, num_chunks, j):
    ...
    or actual_words < int(expected_words * 0.8) or actual_words > int(expected_words * 1.2):
        ...
        print(f"MAX LEN: {summary_len} | ACTUAL LEN: {len(tokenizer(response['choices'][0]['message']['content'])['input_ids'])}")
        ...
        dic[actual_words] = {
            ...
            'response': response,
            'valid_response': response['choices'][0]['message']['content'].strip()[-1] in ['.', '?', '!', '\"', '\''] \
            and len(tokenizer(response['choices'][0]['message']['content'])['input_ids']) < summary_len
        }
...
def get_summaries(path):
    data_path = f'/work/pi_miyyer_umass_edu/yapeichang/books/text/all_books_chunked_{CHUNK_SIZE}.pkl'
    ...
    template_len = len(tokenizer(template.format('', '', 500, 500))['input_ids']) if int(VER[-1]) < 5 else len(tokenizer(template.format('', ''))['input_ids'])
    ...
    for i, book in tqdm.tqdm(enumerate(data), total=len(data), desc="Iterating over books"):
        ...
        for j, chunk in tqdm.tqdm(enumerate(dd), total=len(dd), desc="Iterating over chunks"):
            assert len(tokenizer(chunk)['input_ids']) <= CHUNK_SIZE, f"Chunk {j} in book {i} is too long"
            if j < len(new_chunks):
            ...
            if prev_summary is None:
                prompt = init_template.format(chunk, num_words, num_words) if int(VER[-1]) < 5 else init_template.format(chunk)
            else:
                prompt = template.format(chunk, prev_summary, num_words, num_words) if int(VER[-1]) < 5 else template.format(chunk, prev_summary)
            
            response = obtain_response(prompt, max_tokens=summary_len, temperature=SUMMARY_TEMP)
            print(f"\n\nCHUNK SUMMARY:\n{response['choices'][0]['message']['content'].strip()}\n\n")
            print(f"EXPECTED WORDS: {num_words}")
            actual_words = len(response['choices'][0]['message']['content'].strip().split())
            print(f"ACTUAL WORDS: {actual_words}")
            
            # compress prev_summary if the current one is too long or doesn't end in punctuation
            if prev_summary is not None and (response['choices'][0]['message']['content'].strip()[-1] not in ['.', '?', '!', '\"', '\''] \
            or len(tokenizer(response['choices'][0]['message']['content'])['input_ids']) >= summary_len):
                templates = {
                    "template": template,
            ...
            new_chunk['summary'] = response['choices'][0]['message']['content'].strip()
